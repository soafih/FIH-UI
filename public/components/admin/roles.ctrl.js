angular.module('fihApp').controller('RolesCtrl', function ($scope, $resource, $location, roleList, NgTableParams, $filter, prompt) {
    ...
    $scope.searchUsers = function(){
        ...
        $scope.users = $filter('filter')(roleList.data, {username:$scope.txtUserName, email: $scope.txtEmail});
        
        if($scope.selectedSuperUser !== 'Select'){
            var isSuperUser = false;
            if($scope.selectedSuperUser === 'Yes')
                isSuperUser = true;
            $scope.users = $filter('filter')(roleList.data, {username:$scope.txtUserName, email: $scope.txtEmail, superuser:isSuperUser});
        }
